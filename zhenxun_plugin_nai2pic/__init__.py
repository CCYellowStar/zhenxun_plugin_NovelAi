...
async def runapi(tag: str, W: int, H: int):   
    ...
    seed = response['seed']
    print(seed)
    if Config.get_config("zhenxun_plugin_nai2pic", "DOWNLOAD_nai"):
...
async def translate(_query):
    ...
    if not _appid:
        ...
        return _query
    _salt = Config.get_config("zhenxun_plugin_nai2pic", "salt")
    _key = Config.get_config("zhenxun_plugin_nai2pic", "key")
